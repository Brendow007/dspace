...
public class ResourcePolicy {
    ...
    public static boolean CheckReadPolicyExists(Context context, int id) throws SQLException {

        TableRow row = DatabaseManager.querySingle(context, "select RESOURCE_ID from ResourcePolicy WHERE EPERSONGROUP_ID = 0 AND RESOURCE_ID =" + id);
        if (row == null) {
            return false;
        } else {
            return true;
