...
public class AuthorizeAdminServlet extends DSpaceServlet
{
            ...
            SQLException, AuthorizeException
    {
        ...
        }else if(button.equals("submit_read_policy_anonymous")){


            Item item = Item.find(c, UIUtil.getIntParameter(request, "item_id"));
            Boolean policyExists = ResourcePolicy.CheckReadPolicyExists(c,item.getID());
            if(!policyExists) {
                ...
                ResourcePolicy policyy = ResourcePolicy.find(c, policy.getID());
                AuthorizeUtil.authorizeManagePolicy(c, policyy);
                Group group = Group.find(c, 0);
                if (itemId != -1) {
                    item = Item.find(c, itemId);
...
//               prepItemEditForm(c, request, item);
                    ...
                    response.sendRedirect(response.encodeRedirectURL(request
                            .getContextPath()
                            + "/tools/edit-item?item_id=" + item.getID()));
...
//            String itemIdtoString = Integer.toString(itemId);
//            request.setAttribute(itemIdtoString,"item_id");
