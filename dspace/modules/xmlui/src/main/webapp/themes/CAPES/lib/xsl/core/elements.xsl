...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
    ...
    <xsl:template name="renderHead">
        ...
        <xsl:choose>
            <xsl:when test="not($headerLevel)">
                <xsl:choose>
                    <xsl:when test="$class = 'ds-div-head' and (count(parent::*/preceding-sibling::*) > 0 or count(parent::*/parent::*/preceding-sibling::*) > 0)">
                    ...
                    <xsl:otherwise>
                         <xsl:choose>
                            <xsl:when test="$class = 'ds-list-head'">
                                <div class="outstanding-header-lvl2">
                                    <xsl:element name="h{$head_count}">
                                        <xsl:call-template name="standardAttributes">
                                            <xsl:with-param name="class" select="$class"/>
                                        </xsl:call-template>
    ...
    <xsl:template match="dri:div/dri:head" priority="3">
        <xsl:choose>
            <xsl:when test="not(current()/@level)">
            ...
            <xsl:otherwise>
                <xsl:call-template name="renderHead">
                    <xsl:with-param name="headerLevel"><xsl:value-of select="@level" /></xsl:with-param>
                    <xsl:with-param name="class">ds-div-head</xsl:with-param>
                </xsl:call-template>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template match="dri:table/dri:head" priority="2">
        <xsl:call-template name="renderHead">
            ...
            <xsl:with-param name="headerLevel">3</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="dri:options/dri:list/dri:head" priority="3">
        <xsl:call-template name="renderHead">
            ...
            <xsl:with-param name="headerLevel">2</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
