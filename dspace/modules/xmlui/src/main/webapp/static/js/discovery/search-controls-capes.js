...
(function ($) {
    ...
    function initializeGear(){
        var gearControlsDivs = $('div#aspect_discovery_CapesHomePageSimpleSearch_div_search-controls-gear');
        jQuery.each(gearControlsDivs, function(index, value) {
            var gearControlsDiv = $(value);
            var gearControls = gearControlsDiv.find('ul#aspect_discovery_CapesHomePageSimpleSearch_list_sort-options');
            ...
            gearControls.find('li.gear-option,li.gear-option a').click(function(event){
                ...
                var mainForm = $('form#aspect_discovery_CapesHomePageSimpleSearch_div_main-form');
    ...
    function initializeFilters(){
        ...
        $('a[href="display-filters"]').click(function(){
            var filtersForm = $('form#aspect_discovery_CapesHomePageSimpleSearch_div_search-filters');
            filtersForm.show();
    ...
    function removeFilterRow(button){
        ...
        if(parentRow.is('[id^="aspect_discovery_CapesHomePageSimpleSearch_row_filter-new-"]')
            && parentRow.parents('table:first').find('tr[id^="aspect_discovery_CapesHomePageSimpleSearch_row_filter-new-"]').length == 1){
            //Hide ourselves & clear our values!
            parentRow.find('input[type=text]", select').val('');
        }else{
            if(parentRow.is('[id^="aspect_discovery_CapesHomePageSimpleSearch_row_used-filters-"]') && parentRow.parents('table:first').find('tr[id^="aspect_discovery_CapesHomePageSimpleSearch_row_used-filters-"]').length == 1)
            {
    ...
    function addFilterRow(){
        var previousFilterRow = $('tr#aspect_discovery_CapesHomePageSimpleSearch_row_filter-controls').prev();
