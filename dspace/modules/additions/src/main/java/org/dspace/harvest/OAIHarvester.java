...
public class OAIHarvester {
    ...
    //Integração Identificar a coleção //
            //aplicar o perfil de filtragem de metadados //
                // crud
                ...
                if (records != null && records.size() > 0) {
                    ...
                    for (Element record : records) {
                        ...
                        try {
                            ...
                            log.error("Error processing record " + itemOaiID, e);
                            ourContext.rollback();
                        }
                    }
                }
    ...
    private void processRecord(Element record, String OREPrefix) throws SQLException, AuthorizeException, IOException, CrosswalkException, HarvestingException, ParserConfigurationException, SAXException, TransformerException {
        ...
        // exists in a harvest collection but does not have an OAI_id)
        {
            ...
            if (descMD.size() == 1) {
             // Insert profile here
                MDxwalk.ingest(ourContext, item, descMD.get(0),harvestRow.getCollectionId());
     ...
     * @param MDNamespace the namespace that we are trying to resolve to the
     * metadataPrefix
