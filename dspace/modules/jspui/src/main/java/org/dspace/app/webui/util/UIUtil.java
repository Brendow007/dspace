...
public class UIUtil extends Util
{
    ...
    public static void sendDenunciation(HttpServletRequest request)
    {
        ...
        {
            String recipient = ConfigurationManager
                    .getProperty("denunciation.recipient");

            if (StringUtils.isNotBlank(recipient))
            {
                Email email = Email.getEmail(I18nUtil.getEmailFilename(locale, "denunciation"));
                email.addRecipient(recipient);
                email.addArgument(ConfigurationManager
                        .getProperty("dspace.url"));
                email.addArgument(new Date());
                ...
                email.addArgument(request.getRemoteAddr());
                String nome = request.getParameter("vname");
                String email1 = request.getParameter("vemail");
                String texto = request.getParameter("vmsg");
                String itemid= request.getParameter("vitemid");

                email.addArgument(nome);
                email.addArgument(email1);
                email.addArgument(texto);
                email.addArgument(itemid);

                email.send();

                log.info("Denuncia: item_id:" +itemid+" email: "+email1 );
