    ...
    public int doStartTag() throws JspException {
        ...
        try {
            ...
            for (int i = 0; i < items.length; i++) {
                ...
                if (dcTypeArray != null && dcTypeArray.length > 0) {
                    ...
                    if(fileType.value != null){
                        ...
                        case "áudio":
                            ...
                            break;
                        case "apresentação":
                            icon = "conteudo-apresentacao";
                            break;
                        case "planilha":
                            icon = "conteudo-planilha";
                            break;
                        case "texto":
                            icon = "conteudo-texto";
                            break;
                ...
                out.println("<li><div class=\"thumbnail-wrapper\"><div class=\"artifact-preview\">");
                ...
                for (int colIdx = 0; colIdx < fieldArr.length; colIdx++) {
                    ...
                    // Start metadata value default  if content equal null
                    String metadata = "";
                    if (field.equals("thumbnail")) {
                        ...
                        if (isDate[colIdx]) {
                            ...
                            String value = " - ";
                            try{
                                value = UIUtil.displayDate(dd, false, false, hrq);
                            }catch(Exception e){
                                log.warn("Error trying to convert date " + metadataArray[0].value + ". Convertion ignored!", e);
                            }
                            metadata = value;
...
//                            + "</td>");
                    out.print("<div>" + (emph[colIdx] ? "<strong>" : "") + metadata + (emph[colIdx] ? "</strong>" : "") + "</div>");
                }

                try {
                    out.print("<div>"+items[i].getItemEvaluationHtml()+"</div>");
